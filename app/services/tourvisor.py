...
try:
    ...
    UNIVERSAL_FUZZY_ENABLED = True
except ImportError:
    UNIVERSAL_FUZZY_ENABLED = False
    # Fallback функции
    ...
    def smart_match_region(*args, **kwargs): return None
    RESORT_TYPOS = {}
    DEPARTURE_TYPOS = {}
...
class TourvisorService:
    ...
    async def get_region_id_by_name(self, region_name: str, country_id: int) -> Optional[int]:
        ...
        УЛУЧШЕНО: Использует Universal Fuzzy Matcher для исправления опечаток!
        
        Алгоритм:
        1. Исправление опечаток через fuzzy_matcher (анталия → анталья)
        2. Поиск в regions (list.php?type=region) — ПРИОРИТЕТ!
        3. Поиск в subregions (list.php?type=subregion)
        
        Args:
            region_name: Название региона (напр. "анталия", "пукет")
        ...
        # ==================== ШАГ 0: ИСПРАВЛЕНИЕ ОПЕЧАТОК ====================
        # Используем Universal Fuzzy Matcher для нормализации
        original_name = region_name
        region_name_corrected = normalize_resort(region_name) if UNIVERSAL_FUZZY_ENABLED else region_name
        
        if region_name_corrected != original_name.lower():
            logger.info(f"🔧 TYPO-FIX: '{original_name}' → '{region_name_corrected}'")
        
        # НОРМАЛИЗАЦИЯ: Приводим к нижнему регистру, убираем дефисы → пробелы
        region_name_lower = region_name_corrected.lower().strip()
        region_name_normalized = region_name_lower.replace("-", " ").replace("–", " ")
        
        # ==================== ШАГ 1: Ищем в REGIONS (ПРИОРИТЕТ!) ====================
        # Regions — это крупные области (Анталья, Хургада, Пхукет)
        # Subregions — это мелкие районы внутри (Анталия-центр, Патонг)
        # Важно искать СНАЧАЛА в regions!
        regions = await self.load_regions_for_country(country_id)
        
        # Точное совпадение в regions
        for r in regions:
            r_name_lower = r["name"].lower()
            r_normalized = r_name_lower.replace("-", " ").replace("–", " ")
            
            if (r_name_lower == region_name_lower or 
                r_normalized == region_name_normalized or
                r_name_lower == region_name_normalized or
                r_normalized == region_name_lower):
                logger.info(f"🗺️ Регион '{original_name}' → ID={r['id']} (region, точное: {r['name']})")
                return r["id"]
        
        # Fuzzy совпадение в regions (используем rapidfuzz если доступен)
        if UNIVERSAL_FUZZY_ENABLED:
            result = smart_match_region(region_name_corrected, regions, threshold=80)
            if result:
                logger.info(f"🗺️ Регион '{original_name}' → ID={result[0]} (region, fuzzy: {result[1]}, score={result[2]})")
                return result[0]
        else:
            # Fallback: частичное совпадение
            for r in regions:
                r_normalized = r["name"].lower().replace("-", " ").replace("–", " ")
                if (region_name_normalized in r_normalized or r_normalized in region_name_normalized):
                    logger.info(f"🗺️ Регион '{original_name}' → ID={r['id']} (region, partial: {r['name']})")
                    return r["id"]
        
        # ==================== ШАГ 2: Ищем в SUBREGIONS (fallback) ====================
        # Только если в regions не нашли!
        ...
        for r in subregions:
            ...
            if (r_name_lower == region_name_lower or 
                r_normalized == region_name_normalized):
                logger.info(f"🗺️ Субрегион '{original_name}' → ID={r['id']} (subregion, точное: {r['name']})")
                return r["id"]
        
        # Fuzzy совпадение в subregions
        if UNIVERSAL_FUZZY_ENABLED:
            result = smart_match_region(region_name_corrected, subregions, threshold=80)
            if result:
                logger.info(f"🗺️ Субрегион '{original_name}' → ID={result[0]} (subregion, fuzzy: {result[1]}, score={result[2]})")
                return result[0]
        else:
            # Fallback: частичное совпадение
            for r in subregions:
                r_normalized = r["name"].lower().replace("-", " ").replace("–", " ")
                if (region_name_normalized in r_normalized or r_normalized in region_name_normalized):
                    logger.info(f"🗺️ Субрегион '{original_name}' → ID={r['id']} (subregion, partial: {r['name']})")
                    return r["id"]
        
        logger.warning(f"⚠️ Регион/субрегион '{original_name}' (corrected: '{region_name_corrected}') не найден в стране {country_id}")
        return None
    ...
    def _score_hotel_relevance(self, hotel_name: str, original_query: str) -> int:
        """
        РЕШЕНИЕ 2: Сортировка по релевантности.
        Подсчитывает "очки релевантности" отеля к запросу пользователя.
        
        Логика подсчёта:
        - Каждое слово запроса, найденное в названии отеля = +10 очков
        - Точное совпадение запроса как подстроки = +50 очков (бонус)
        
        Пример: запрос "Titanic Deluxe Belek"
        - TITANIC DELUXE GOLF BELEK → 3 слова (30) + бонус за "belek" → 30+ очков
        - TITANIC DELUXE LARA → 2 слова (20) → 20 очков
        """
        ...
        for word in query_words:
            if len(word) >= 2 and word in hotel_lower:  # Игнорируем слишком короткие слова
                score += 10
        
        # Бонус за точное совпадение запроса как подстроки
        if query_lower in hotel_lower:
            score += 50
        
        # Бонус за совпадение в начале названия (более релевантно)
        if hotel_lower.startswith(query_words[0]) if query_words else False:
            score += 20
        ...
        for cid in search_country_ids:
            ...
            for hotel in hotels:
                ...
                for variant in search_variants:
                    # РЕШЕНИЕ 1 FIX: Два режима поиска
                    # 1. Точное вхождение подстроки (как раньше)
                    # 2. Пословное совпадение (все слова запроса есть в названии)
                    
                    match_found = False
                    
                    # Режим 1: Точное вхождение
                    if variant in hotel_name_lower:
                        match_found = True
                    
                    # Режим 2: Пословное совпадение (fallback)
                    # "titanic deluxe belek" → проверяем что все 3 слова есть в названии
                    if not match_found and len(variant.split()) >= 2:
                        variant_words = set(variant.split())
                        hotel_words = set(hotel_name_lower.split())
                        # Все слова из запроса должны быть в названии отеля
                        if variant_words.issubset(hotel_words):
                            match_found = True
        ...
        if region_lower or resort_lower:
            if filtered_by_region:
                ...
                sorted_results = sorted(
                    filtered_by_region, 
                    key=lambda h: self._score_hotel_relevance(h.name, query), 
                    reverse=True
                )
                if len(sorted_results) > 1:
                    logger.info(f"   🎯 Отсортировано по релевантности: {sorted_results[0].name} (score={self._score_hotel_relevance(sorted_results[0].name, query)})")
    ...
    async def search_tours(
        ...
        # ШАГ 2 FIX: Для hotel_only режима departure_city не нужен!
        mode = getattr(params, "search_mode", "package")
        is_hotel_only = (mode == "hotel_only")
        
        if is_hotel_only:
            departure_id = None  # Будет 0 в API
            logger.info(f"   🏨 HOTEL_ONLY MODE: город вылета не требуется")
        else:
        ...
        # FIX-RESORT-API: Ищем по region ИЛИ resort (курорт)!
        # Ранее проверялся только region, а resort игнорировался.
        # FIX-REGION-PRIORITY: Приоритет resort над невалидным region!
        # Если region = "не указан"/"null"/"none" — игнорируем его и берём resort.
        region_id = None
        if params.destination and (params.destination.region or params.destination.resort):
            # FIX-REGION-PRIORITY: Фильтруем невалидные значения region
            INVALID_REGION_VALUES = ("не указан", "не указано", "null", "none", "", "нет")
            region_val = params.destination.region
            resort_val = params.destination.resort
            country_name = params.destination.country or ""
            
            # Приоритет: валидный region, иначе resort
            if region_val and region_val.lower().strip() not in INVALID_REGION_VALUES:
                search_region_name = region_val
            elif resort_val and resort_val.lower().strip() not in INVALID_REGION_VALUES:
                search_region_name = resort_val
            else:
                search_region_name = None
                logger.warning(f"   ⚠️ FIX-REGION-PRIORITY: region='{region_val}', resort='{resort_val}' — оба невалидны")
            
            # ==================== FIX CASE-003 ====================
            # Если resort/region совпадает с названием страны → НЕ искать регион!
            # "Доминикана" как регион внутри "Доминикана" — бессмысленно и ломает API
            if search_region_name and country_name:
                if search_region_name.lower().strip() == country_name.lower().strip():
                    logger.info(f"   🛡️ FIX CASE-003: resort='{search_region_name}' == country='{country_name}' → пропускаем поиск региона")
                    search_region_name = None  # Не искать регион — поиск по всей стране
            
            if search_region_name:
                logger.info(f"   🗺️ FIX-RESORT-API: Поиск региона/курорта '{search_region_name}'")
                region_id = await self.get_region_id_by_name(
                    search_region_name, 
                    country_id
                )
                if region_id:
                    logger.info(f"   ✅ FIX-RESORT-API: Найден ID={region_id} для '{search_region_name}'")
                else:
                    logger.warning(f"   ⚠️ FIX-RESORT-API: Регион '{search_region_name}' не найден в справочнике")
        ...
        try:
            ...
            offers = await self._poll_and_fetch_results(
                ...
                is_hotel_only=is_hotel_only  # FIX-FRONTEND: передаём режим hotel_only
            )
            ...
            # ==================== FIX 2.3: СТРОГАЯ ФИЛЬТРАЦИЯ ПО ТИПУ ПИТАНИЯ ====================
            # API Tourvisor интерпретирует meal как МИНИМАЛЬНЫЙ уровень, а не ТОЧНЫЙ!
            # Например, meal=4 (HB) может вернуть HB, FB, AI, UAI.
            # ОБЯЗАНЫ фильтровать на своей стороне для соответствия запросу клиента.
            
            if params.food_type and offers:
                requested_food = params.food_type.value if hasattr(params.food_type, 'value') else str(params.food_type)
                original_count = len(offers)
                
                # СТРОГИЙ ФИЛЬТР: оставляем ТОЛЬКО туры с запрошенным типом питания
                filtered_by_food = [
                    o for o in offers 
                    if o.food_type and (
                        o.food_type.value if hasattr(o.food_type, 'value') else str(o.food_type)
                    ) == requested_food
                ]
                
                if filtered_by_food:
                    logger.info(f"   🍽️ STRICT FOOD FILTER {requested_food}: было {original_count}, стало {len(filtered_by_food)}")
                    offers = filtered_by_food
                else:
                    # FIX 2.3: Если с запрошенным питанием туров нет — возвращаем результат с альтернативами!
                    logger.warning(f"   ⚠️ FOOD FILTER: Туров с {requested_food} нет из {original_count}.")
                    
                    # Собираем доступные типы питания для информирования
                    available_foods = set()
                    for o in offers:
                        if o.food_type:
                            food_val = o.food_type.value if hasattr(o.food_type, 'value') else str(o.food_type)
                            available_foods.add(food_val)
                    
                    if available_foods:
                        logger.info(f"   🍽️ Доступные типы питания: {sorted(available_foods)}")
                        # Возвращаем пустой результат с указанием доступных альтернатив
                        return SearchResponse(
                            offers=[],
                            total_found=0,
                            search_id=request_id,
                            found=False,
                            reason="no_tours_with_food_type",  # Специальный reason для nodes.py
                            suggestion=",".join(sorted(available_foods))  # Доступные типы питания
                        )
                    
                    offers = []  # КРИТИЧНО: обнуляем для активации fallback!
            
            # Сортируем по приоритету ночей, затем по цене, берём СТРОГО 5 (лимит Pydantic!)
            offers = self._sort_offers_by_nights_priority(offers, params.nights)[:5]
            ...
            if offers:
                ...
                # fix-resort-api FALLBACK: Если туры не найдены И был указан region — повторяем без региона
                if region_id and not getattr(self, '_region_fallback_attempted', False):
                    logger.warning(f"   🔄 fix-resort-api FALLBACK: 0 туров с region={region_id}, повторяем без региона...")
                    self._region_fallback_attempted = True
                    
                    # Повторный поиск без региона
                    fallback_params = self._build_search_params(
                        params, country_id, departure_id, hotel_ids,
                        is_hot_tour=is_hot_tour,
                        region_id=None  # БЕЗ региона!
                    )
                    
                    try:
                        init_result = await self._request("search.php", fallback_params)
                        fallback_request_id = str(init_result.get("data", {}).get("requestid", ""))
                        
                        if fallback_request_id:
                            # Повторяем цикл ожидания
                            for attempt in range(self._search_max_attempts):
                                await asyncio.sleep(self._search_poll_interval)
                                status_result = await self._request("result.php", {
                                    "type": "status",
                                ...
                                if progress > 0:
                                    result_data = await self._request("result.php", {
                                        "type": "result",
                                        "requestid": fallback_request_id
                                    })
                                    
                                    fallback_offers = self._parse_offers(result_data)
                                    if fallback_offers:
                                        fallback_offers = self._sort_offers_by_nights_priority(fallback_offers, params.nights)[:5]
                                        logger.info(f"   ✅ FALLBACK успешен: {len(fallback_offers)} туров (без региона)")
                                        self._region_fallback_attempted = False
                                        return SearchResponse(
                                            offers=fallback_offers,
                                            total_found=len(fallback_offers),
                                            search_id=fallback_request_id,
                                            found=True,
                                            suggestion=f"Туры в регионе {params.destination.region or params.destination.resort} не найдены, показаны по всей стране"
                                        )
                                    break
                    except Exception as e:
    ...
    def _build_search_params(
        ...
        # ШАГ 2 FIX: Для hotel_only используем Москву (ID=1) как дефолт
        # Tourvisor API НЕ поддерживает departure=0 — возвращает 0 результатов!
        # Но пользователю мы НЕ показываем/спрашиваем город вылета.
        if is_hotel_only:
            final_departure_id = 1  # Москва — максимум вылетов и предложений
            logger.info(f"   🚗 HOTEL_ONLY MODE: departure=1 (Москва-дефолт, не показываем клиенту)")
        elif departure_id:
        ...
        api_params = {
            ...
            # fix-hotelonly: Для hotel_only режима hideregular=0 (неважно — без перелёта)
            # Для package/burning — hideregular=1 (только чартерные рейсы)
            "hideregular": 0 if is_hotel_only else 1,
        }
        ...
        # FIX-RESORT-API: Передаём region_id (найденный выше из region ИЛИ resort)
        # FIX-REGION-PRIORITY-v2: Фильтруем невалидные значения и в fallback!
        INVALID_REGION_VALUES = ("не указан", "не указано", "null", "none", "", "нет")
        
        if region_id:
            api_params["regions"] = region_id  # CRITICAL: "regions", не "region"!
            # Для лога берём валидное имя
            region_val = params.destination.region if params.destination else None
            resort_val = params.destination.resort if params.destination else None
            if region_val and region_val.lower().strip() not in INVALID_REGION_VALUES:
                region_name = region_val
            elif resort_val and resort_val.lower().strip() not in INVALID_REGION_VALUES:
                region_name = resort_val
            else:
                region_name = "?"
            logger.info(f"   🗺️ Регион/курорт '{region_name}' → regions={region_id}")
        elif params.destination:
            # Fallback: передаём текстом (не рекомендуется, но API может понять)
            # FIX-REGION-PRIORITY-v2: Применяем ту же фильтрацию!
            region_val = params.destination.region
            resort_val = params.destination.resort
            country_name = params.destination.country or ""
            ...
            if region_val and region_val.lower().strip() not in INVALID_REGION_VALUES:
                ...
                region_name = None  # Оба невалидны — не передаём regions!
            
            # ==================== FIX CASE-003 (fallback) ====================
            # Если resort/region == country → не передавать regions!
            if region_name and country_name:
                if region_name.lower().strip() == country_name.lower().strip():
                    logger.info(f"   🛡️ FIX CASE-003: resort='{region_name}' == country → не передаём regions")
                    region_name = None
            
            if region_name:
                api_params["regions"] = region_name
                logger.warning(f"   ⚠️ Регион '{region_name}' передаём текстом (ID не найден)")
            else:
                logger.info(f"   ℹ️ Регион не указан — поиск по всей стране")
    ...
    async def _poll_and_fetch_results(
        ...
        is_hotel_only: bool = False  # FIX-FRONTEND: режим "только проживание"
    ) -> list[TourOffer]:
        """
        ...
        for attempt in range(1, self.max_poll_attempts + 1):
            ...
            if status.state == "finished":
                final_offers = await self._fetch_results(
                    request_id, country_id, is_strict_hotel_search, hotel_ids,
                    onpage=onpage, departure_city=departure_city, is_hotel_only=is_hotel_only
                )
                if final_offers:
    ...
    async def _fetch_results(
        ...
        departure_city: str = "Москва",  # Город вылета для карточек
        is_hotel_only: bool = False  # FIX-FRONTEND: для режима "только проживание"
    ) -> list[TourOffer]:
        """
        ...
        Args:
            ...
            is_hotel_only: Режим "только проживание" (без перелёта)
        """
        try:
            response = await self._request("result.php", {
                "type": "result",
            ...
            return self._parse_tour_results(
                response, country_id, is_strict_hotel_search, hotel_ids, departure_city, is_hotel_only
            )
        except Exception as e:
    ...
    async def continue_search(
        ...
        country_id: int,
        departure_city: str = "Москва",  # Город вылета для карточек
        requested_nights: Optional[int] = None  # Запрошенное количество ночей для сортировки
        ...
        Args:
            ...
            country_id: ID страны
            departure_city: Город вылета (для отображения в карточках)
            requested_nights: Запрошенное количество ночей (для приоритетной сортировки)
        ...
        try:
            ...
            # Сортируем по приоритету ночей, затем по цене, берём СТРОГО 5 туров (лимит Pydantic!)
            offers = self._sort_offers_by_nights_priority(offers, requested_nights)[:5]
    ...
    def _parse_tour_results(
        ...
        for hotel in hotels_data:
            ...
            try:
                offer = self._parse_single_offer(hotel, expected_country, departure_city, is_hotel_only)
                if offer:
                    offers.append(offer)
            except Exception as e:
    ...
    def _parse_single_offer(
        ...
        return TourOffer(
            ...
            # FIX-FRONTEND: Для hotel_only — нет перелёта
            flight_included=not is_hotel_only,
            is_hotel_only=is_hotel_only,
        )
    ...
    def _sort_offers_by_nights_priority(
        self, 
        offers: list[TourOffer], 
        requested_nights: Optional[int]
    ) -> list[TourOffer]:
        """
        Сортировка туров: приоритет точному соответствию ночам, затем по цене.
        
        Если requested_nights=7:
        - Сначала все туры на 7 ночей (по цене)
        - Затем все остальные (по цене)
        
        Args:
            offers: Список туров для сортировки
            requested_nights: Запрошенное количество ночей (None = только по цене)
        
        Returns:
            Отсортированный список туров
        """
        if requested_nights is None:
            # Нет запроса по ночам — только по цене
            return sorted(offers, key=lambda x: x.price)
        
        # Логируем для отладки
        exact_match = sum(1 for o in offers if o.nights == requested_nights)
        other = len(offers) - exact_match
        logger.info(f"   🔢 NIGHTS SORT: requested={requested_nights}, exact_match={exact_match}, other={other}")
        
        return sorted(offers, key=lambda x: (
            0 if x.nights == requested_nights else 1,
            x.price
        ))
    ...
    def _parse_hot_tour(self, tour: dict, departure_city: str = "Москва") -> Optional[TourOffer]:
        ...
        # P4 FIX: Парсим реальный тип питания из API
        # hottours.php может возвращать meal как ID (int) или как код (str)
        meal_raw = tour.get("meal") or tour.get("mealtype") or tour.get("mealname", "")
        food_type = FoodType.AI  # По умолчанию
        
        if meal_raw:
            meal_str = str(meal_raw).upper()
            # Проверяем известные коды
            if any(code in meal_str for code in ["AI", "UAI", "ALL"]):
                food_type = FoodType.AI
            elif any(code in meal_str for code in ["FB", "FULL"]):
                food_type = FoodType.FB
            elif any(code in meal_str for code in ["HB", "HALF"]):
                food_type = FoodType.HB
            elif any(code in meal_str for code in ["BB", "BREAK"]):
                food_type = FoodType.BB
            elif any(code in meal_str for code in ["RO", "ROOM", "WITHOUT"]):
                food_type = FoodType.RO
            # Если числовой ID — используем MEAL_TYPE_REVERSE
            elif meal_str.isdigit():
                meal_code = MEAL_TYPE_REVERSE.get(int(meal_str))
                if meal_code:
                    food_type = FoodType(meal_code)
        ...
        return TourOffer(
            ...
            room_type=tour.get("room", "Standard"),
            food_type=food_type,  # P4 FIX: Используем реальное значение
            price=int(price),
            currency="RUB",
            date_from=date_from,
            date_to=date_from + timedelta(days=nights),
